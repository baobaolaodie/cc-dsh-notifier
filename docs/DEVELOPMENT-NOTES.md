...
- **spikes 实验脚本**:`scripts/spikes/`(23 个文件,含 AUMID/激活器/lnk/协议/COM 五条路径调试记录)、`recover.py`、`test-fixture-*.json` 曾恢复在磁盘但**不入库**(.gitignore 已排除);2026-08-18 清理阶段已删除,仅保留 `.gitignore` 规则防止再次误入库。
...
- **测试**:114 项;`npm test` 显式清单。Python/PS 脚本无自动化测试(真实环境手动回归)。窗口绑定决策已抽到 `scripts/lib/bind-window.mjs`(纯函数 + 依赖注入)并有**真实场景矩阵单测**(`test/bind-window.test.mjs`,9 用例:VSCode/独立终端 × dsh-tui/Claude/web/多会话不串窗/多 ? 并存/早于标题),`?` 绑定/懒绑定/console-hwnd 均有测试覆盖(2026-08-18)。
