...
- **安装(三种方式)**:①tarball `add ./baobaolaodie-cc-dsh-notifier-0.1.4.tgz`(零配置、不 clone 仓库、**最方便**,生产采用);②git `add github:baobaolaodie/cc-dsh-notifier`(公开零配置,但会 clone 整个仓库);③GitHub Packages `add @baobaolaodie/cc-dsh-notifier`(个人便捷渠道,**下载始终需要 token 即使 public**:profile/.npmrc 配 `@baobaolaodie:registry=https://npm.pkg.github.com` + `//npm.pkg.github.com/:_authToken=<PAT read:packages>`)。Windows 跨盘 junction 缺陷兜底(仅 link 安装):修 junction + `dsh plugin --profile <name> list`。dump-config 验证 `# == <包名>` 层。
