...
3. **Window is unfocused** — notifications are suppressed while the session window is focused (by design).
4. **AUMID registered** — re-run `node scripts/install.mjs`; verify with:
   ```
   reg query HKCU\Software\Classes\AppUserModelId\cc-notifier
   ```
5. **Python + winrt present** — `python -c "import winrt.windows.ui.notifications"` must succeed.
6. **Daemon running** — `%LOCALAPPDATA%\cc-notifier\daemon.json` exists and the pid is alive. Check `daemon.log`.
