...
function main() {
  if (!fs.existsSync(SETTINGS)) {
    console.log('settings.json 不存在,无需卸载 / settings.json does not exist, nothing to uninstall');
    unregisterAumid();
    return;
  }
  let settings;
  ...
  if (fs.existsSync(BACKUP)) {
    ...
    console.log('已从备份恢复 settings.json / restored settings.json from backup');
  } else {
    fs.writeFileSync(SETTINGS, JSON.stringify(settings, null, 2) + '\n');
    console.log('已移除注入的 hooks 条目 / removed injected hook entries');
  }

  unregisterAumid();

  console.log('卸载完成。daemon 将在会话空 + 60s 后自动退出;如需立即停止,关闭全部 Claude Code 会话即可。 / Uninstall complete. The daemon exits 60s after the last session ends; close all Claude Code sessions to stop it now.');
}
